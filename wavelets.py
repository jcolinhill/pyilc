...
def wavelet_ILC(wv=None, info=None, ILC_bias_tol=1.e-3, wavelet_beam_criterion=1.e-3, resp_tol=1.e-3, map_images=False):
    ...
    for j in range(wv.N_scales):
        ...
        for a in range(info.N_freqs):
            if (freqs_to_use[j][a] == True):
                weight_filename = info.output_dir+info.output_prefix+'weightmap_freq'+str(a)+'_scale'+str(j)+'_component_'+info.ILC_preserved_comp+info.ILC_preserved_comp+'_deproject_'+'_'.join(info.ILC_deproj_comps)+'.fits' #Fiona edit: it is useful to include all deprojected components in the filename
        ...
        if (weights_exist == False):
            ...
            if (info.save_weights == 'yes' or info.save_weights == 'Yes' or info.save_weights == 'YES'):
                ...
                for a in range(info.N_freqs):
                    if (freqs_to_use[j][a] == True):
                        weight_filename = info.output_dir+info.output_prefix+'weightmap_freq'+str(a)+'_scale'+str(j)+'_component_'+info.ILC_preserved_comp+'_deproject_'+'_'.join(info.ILC_deproj_comps)+'.fits'#Fiona edit: it is useful to include all deprojected components in the filename
                        ...
                        if map_images == True: #save images if requested
                            ...
                            plt.savefig(info.output_dir+info.output_prefix+'_needletILCweightmap_freq'+str(a)+'_scale'+str(j)+'_component_'+info.ILC_preserved_comp+'_deproject_'+'_'.join(info.ILC_deproj_comps)+'.pdf')#Fiona edit: it is useful to include all deprojected components in the filename
                        count+=1
        else:
            weights = np.zeros((int(N_pix_to_use[j]),int(N_freqs_to_use[j])))
            count=0
    ...
    ILC_map_filename = info.output_dir+info.output_prefix+'needletILCmap'+'_component_'+info.ILC_preserved_comp+'_deproject_'+'_'.join(info.ILC_deproj_comps)+'.fits' #Fiona edit: it is useful to include all deprojected components in the filename
